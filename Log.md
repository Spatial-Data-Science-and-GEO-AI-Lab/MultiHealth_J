...
||- Letting the script run the whole day to retrieve the statistics for each grid street network. Will further work on extracting the NDVI and LST values. Will also work on further optimizing the code since the multiprocessing does not seem to work for the distance calculations.
