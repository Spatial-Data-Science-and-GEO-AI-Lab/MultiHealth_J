...
||-  Finished adding all the variables and merged with the distance variables that Sander did. Also tried adding network statistics for the rows that did have none due to not having a nearby street networks or no viable area, but this did not help so those rows remain NaN.
